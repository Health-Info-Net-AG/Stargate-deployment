...
MODE="${1:?usage: generate-release-notes.sh milestone <tag>}"
TAG="${2:?usage: generate-release-notes.sh milestone <tag>}"

PROD_FILE="docker-compose/customer-config-prod.example.sh"
PREPROD_FILE="docker-compose/customer-config-preprod.example.sh"

SEMVER_RE='^v[0-9]+\.[0-9]+\.[0-9]+$' # v1.0.0
...
case "$MODE" in
  milestone)
    ...
    # Roll up since the previous semver tag, ignoring the historic CalVer
    # build tags (vYYYY.MM.DD-HHMM) left by the removed release_build job.
    PREV_TAG="$(prev_tag "$REF" "$SEMVER_RE")"
    ;;
  *)
    echo "unknown mode: ${MODE} (expected milestone)" >&2
    exit 1
    ;;
esac
