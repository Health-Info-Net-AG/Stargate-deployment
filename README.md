...
```shell
docker run --rm -it -p 8000:8000 --entrypoint /bin/sh -v ${PWD}:/docs squidfunk/mkdocs-material -c "pip install mkdocs-glightbox mkdocs-print-site-plugin && mkdocs build -f config_docs/en/mkdocs.yml ; mkdocs build -f config_docs/de/mkdocs.yml ; mkdocs build -f config_docs/fr/mkdocs.yml ; mkdocs build -f config_docs/it/mkdocs.yml"
```
