...
* Ensure there is an internet connection on the machine where you are installing HIN Gateway services
* Ensure traffic is properly configured to reach the HIN Gateway instance
...
| `POLICY_SYNC_REPO_URL` | GitHub HIN HIN Gateway policies | Git repo URL for OPA/Rego policy sync |
...
The S/MIME CSR submission on `/onboarding` will fail if your HIN Gateway instance is not yet registered as a WireGuard peer on the HIN CA side. This is the most common issue during initial setup.
...
3. **`SERVER_STATIC_IP`** — the public IP of your HIN Gateway server
...
!!! tip
    Check your firewall: Port `19818/TCP` must be open **both directions inbound and outbound** on the HIN Gateway server.
...
The HIN Gateway sends mail from its own public IP on behalf of your users. Without proper DNS authentication records, recipients will see "we can't verify this sender" warnings and may reject the mail.
...
At minimum, for each domain you route through the HIN Gateway:

* **SPF**: add `ip4:<HIN_GATEWAY_IP>` to the domain's TXT record
* **DMARC**: publish `v=DMARC1; p=none` at `_dmarc.<YOUR_DOMAIN>`
* **PTR**: set reverse DNS for the HIN Gateway IP to match `MAIL_HOSTNAME`

### Step 6.2 Relay outbound mail back through your mail platform (recommended for M365 / Exchange Online)

By default, after the HIN Gateway signs/encrypts an outbound mail it delivers directly to the recipient's MX. This works, but the connecting IP is your HIN Gateway's IP - and unless that IP has years of warm reputation, it can end up on third-party blocklists (e.g. Barracuda, Abusix), causing intermittent delivery failures.

The recommended pattern is to **send the signed mail back through your M365 / Exchange tenant** so that the final hop to the internet is Microsoft's well-reputed infrastructure. The HIN Gateway still signs and policy-checks every message; only the last hop changes. This mirrors the original HIN MGW "Send to MX" connector pattern.

#### HIN Gateway side — per-domain relay
...
1. **Inbound connector** - accept mail from the HIN Gateway, identified by TLS certificate (the cert subject must match a domain accepted in your tenant). A self-signed cert on the HIN Gateway will be rejected by this connector - use a valid CA-issued cert (Let's Encrypt is fine).
...
4. **Transport rule `outgoing_to_mx`** - matches the `outgoing_<domain>` header on mail coming back from the HIN Gateway and routes it via the "Send to MX" connector.
5. **Transport rule `mgw_bypass_antispam`** - bypasses spam filtering on mail coming back from the HIN Gateway.
