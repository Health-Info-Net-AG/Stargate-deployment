...
1. **Routing**: Select **"Route email through these smart hosts"**
   - Enter the HIN Gateway server IP address: `<HIN_GATEWAY_IP>`
   - Click **"+"** to add it, then click **"Next"**

2. **Security restrictions**: Select **"Any digital certificate, including self-signed certificates"**
   - Click **"Next"**
...
1. **Validation email**: Enter a valid email address for your domain (e.g. `user@<YOUR_DOMAIN>`)
...
1. Review the settings and click **"Create connector"**

2. On the confirmation screen, click **"Done"**
...
1. **Do the following**: Select **"Redirect the message to..."** → **"the following connector"** → select the outbound connector created in Step A (e.g. "From Office 365 to HIN Gateway relay server")
   - Click **"Save"**

2. **Except if**: Click **"+"** to add an exception
...
1. Review the rule summary. It should show:
...
2. Click **"Next"**, then **"Next"** again, then **"Finish"**, then **"Done"**

3. **Enable the rule**: The rule is created in a disabled state. Click on the rule in the list and toggle **"Enable or disable rule"** to **"Enabled"**
