...
- Click **+ Create port rule**, Inbound port rule, Destination port ranges 25, Protocol TCP, name it SMTP. Repeat for the other required inbound ports — **8084** (seal callback) and **19818** (WireGuard). See [Server Requirements → Inbound Network Access](../index.md#inbound-network-access-firewall-must-allow) for the full list.
