# Configuration DNS pour HIN Gateway

Ce guide couvre tous les enregistrements DNS requis pour un déploiement fonctionnel de HIN Gateway. Configurez ces enregistrements **avant** d'installer HIN Gateway ou immédiatement après, selon le type d'enregistrement.

Tout au long de ce guide:

- `<HIN_GATEWAY_IP>` - l'adresse IP publique statique de votre serveur HIN Gateway (`SERVER_STATIC_IP` dans `customer-config.sh`)
- `<MAIL_HOSTNAME>` - le FQDN du relais HIN Gateway (ex. `mail.example.ch` ; configuré via la page `/mail` du tableau de bord)
...
| -------- | ------ | ------- | ---------- | ------ |
| [A](#enregistrement-a) | `<MAIL_HOSTNAME>` | `<HIN_GATEWAY_IP>` | Oui | Avant l'installation |
| [MX](#enregistrements-mx) | `<YOUR_DOMAIN>` | `<MAIL_HOSTNAME>` (priorité 15) | Oui | Avant l'installation |
| [SPF](#enregistrement-spf) | `<YOUR_DOMAIN>` | `ip4:<HIN_GATEWAY_IP>` ajouté au TXT | Oui | Avant l'installation |
| [PTR](#ptr-dns-inverse) | `<HIN_GATEWAY_IP>` | `<MAIL_HOSTNAME>` | Recommandé | Avant l'installation |
...
Créez un enregistrement A pointant le nom d'hôte du courrier HIN Gateway vers l'IP publique du serveur:
...
Si HIN Gateway dispose d'une adresse IPv6, ajoutez également un enregistrement AAAA:
...
Ajoutez un enregistrement MX pour HIN Gateway avec une **priorité plus élevée** (nombre inférieur) que le serveur de courrier existant. Cela garantit que les courriels entrants atteignent d'abord HIN Gateway pour le traitement S/MIME avant d'être transférés vers Exchange ou votre plateforme de courrier.
...
!!! info
    Un numéro MX inférieur signifie une priorité plus élevée. HIN Gateway avec la priorité 15 reçoit les courriels avant Exchange Online avec la priorité 20.

**Pourquoi**: HIN Gateway intercepte les courriels entrants, traite S/MIME, puis les transfère au prochain MX (Exchange). Le deuxième enregistrement MX est également utilisé par Stalwart pour savoir où relayer les courriels traités.

**Important**: Si HIN Gateway est le **seul** enregistrement MX pour un domaine, Stalwart filtrera son propre nom d'hôte et n'aura pas de cible de livraison. Conservez toujours un deuxième MX pointant vers votre serveur de courrier réel.

### Enregistrement SPF

Ajoutez l'IP du serveur HIN Gateway **et l'IP du scelleur HIN** à l'enregistrement SPF de votre domaine afin que les courriels sortants relayés via celui-ci réussissent les vérifications SPF chez le destinataire.
...
**Comment HIN Gateway utilise SPF**: Le démon mtaconf résout l'enregistrement SPF de chaque domaine pour remplir automatiquement la liste des IP autorisées à relayer via HIN Gateway sans authentification. C'est ainsi que les IP sortantes de Microsoft 365 sont automatiquement mises sur liste blanche - elles apparaissent dans la chaîne `include:spf.protection.outlook.com`.
...
Configurez l'enregistrement DNS inversé (PTR) pour l'IP HIN Gateway afin qu'il corresponde à `<MAIL_HOSTNAME>`:
...
| SPF incluant `ip4:<HIN_GATEWAY_IP>` | Oui |
| DMARC (`_dmarc.<domain>`) | Recommandé |
| DKIM (de votre fournisseur de courrier) | Recommandé |

L'enregistrement A et l'enregistrement PTR sont partagés (ils pointent vers le serveur HIN Gateway, pas vers des domaines individuels).

### Routage de courrier par domaine

Les enregistrements MX de chaque domaine indiquent à HIN Gateway où livrer les courriels traités. Si différents domaines utilisent différents serveurs Exchange:
...
# Enregistrement A
host <MAIL_HOSTNAME>
# Attendu: <MAIL_HOSTNAME> a l'adresse <HIN_GATEWAY_IP>

# Enregistrements MX
host -t mx <YOUR_DOMAIN>
# Attendu: Les enregistrements MX de HIN Gateway et d'Exchange sont répertoriés

# Enregistrement SPF
host -t txt <YOUR_DOMAIN> | grep v=spf1
# Attendu: L'enregistrement SPF inclut ip4:<HIN_GATEWAY_IP>

# PTR (DNS inversé)
host <HIN_GATEWAY_IP>
# Attendu: <HIN_GATEWAY_IP> → <MAIL_HOSTNAME>

# DNS inversé à confirmation directe (FCrDNS)
host $(host <HIN_GATEWAY_IP> | awk '{print $NF}' | sed 's/\.$//')
# Attendu: résout en <HIN_GATEWAY_IP>
...
- SPF est manquant ou n'inclut pas l'IP HIN Gateway - ajoutez `ip4:<HIN_GATEWAY_IP>` à votre enregistrement SPF
...
### La recherche MX ne renvoie que HIN Gateway

Si HIN Gateway est le seul MX pour un domaine, Stalwart filtre son propre nom d'hôte et n'a pas de cible de relais. Ajoutez un deuxième enregistrement MX pointant vers votre serveur de courrier:

```plain
example.ch.    MX    15    mail.example.ch.          ← HIN Gateway (entrant)
